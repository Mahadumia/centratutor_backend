...
// Create models
const WaecSubject = mongoose.model('WaecSubject', subjectSchema);
const WaecYear = mongoose.model('WaecYear', yearSchema);
const WaecTopic = mongoose.model('WaecTopic', topicSchema);

class WaecSubjectModel {
  constructor() {
  ...
  async getAllSubjects() {
    try {
      const subjects = await WaecSubject.find().sort({ name: 1 });
  ...
  async getSubjectById(id) {
    try {
      const subject = await WaecSubject.findById(id);
  ...
  async getYearsBySubject(subjectName) {
    try {
      ...
      const yearDoc = await WaecYear.findOne({ subjectKey });
  ...
  async getTopicsBySubjectAndYear(subjectName, year) {
    try {
      const subjectKey = this._getSubjectKey(subjectName);
      const topicDoc = await WaecTopic.findOne({ subjectKey, year });
  ...
  async getAllTopicsBySubject(subjectName) {
    try {
      const subjectKey = this._getSubjectKey(subjectName);
      const topicDocs = await WaecTopic.find({ subjectKey });
  ...
  async addSubject(subject) {
    try {
      const newSubject = new WaecSubject({
        name: subject.name,
      ...
      await WaecYear.findOneAndUpdate(
        { subjectKey },
        { subjectKey, years: [] },
  ...
  async addYearsToSubject(subjectName, years) {
    try {
      ...
      const yearDoc = await WaecYear.findOneAndUpdate(
        { subjectKey },
        { subjectKey },
        { upsert: true, new: true }
      );
  ...
  async addTopicsToSubjectYear(subjectName, year, topics) {
    try {
      ...
      const topicDoc = await WaecTopic.findOneAndUpdate(
        { subjectKey, year },
        { subjectKey, year },
        { upsert: true, new: true }
      );
  ...
  async updateSubject(id, updates) {
    try {
      ...
      if (updates.name && updates.name !== subject.name) {
        ...
        await WaecYear.updateOne(
          { subjectKey: oldKey },
          { subjectKey: newKey }
        );
        
        // Update topics collection
        await WaecTopic.updateMany(
          { subjectKey: oldKey },
          { subjectKey: newKey }
        );
      }
      ...
      // Update the subject
      const updatedSubject = await WaecSubject.findByIdAndUpdate(
        id,
        updates,
        { new: true }
      );
  ...
  async deleteSubject(id) {
    try {
      ...
      // Delete the subject
      await WaecSubject.findByIdAndDelete(id);
      
      // Clean up related data
      await WaecYear.deleteOne({ subjectKey });
      await WaecTopic.deleteMany({ subjectKey });
  ...
  async getFullData() {
    try {
      ...
      // Get all years
      const yearDocs = await WaecYear.find();
      const years = {};
      ...
      // Get all topics
      const topicDocs = await WaecTopic.find();
      const topics = {};
...
module.exports = WaecSubjectModel;